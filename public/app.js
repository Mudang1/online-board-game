import {renderEstate} from './estate-ui.js';
...
function render(){
 ...
 const boom=r.mode==='boom',crystal=r.mode==='crystal',estate=r.mode==='estate';
 $('#esBoard').classList.toggle('hidden',!estate);
 $('#cgBoard').classList.toggle('hidden',!crystal);
 $('.table-shell').classList.toggle('hidden',crystal||estate);
 $('.self-panel').classList.toggle('hidden',crystal||estate);
 ...
 if(estate){$('#peekPanel').classList.add('hidden');renderEstate(state,{canAct:enabled&&myTurn,send:payload=>act('estate-action',payload),esc});showWinner(r,p,enabled);tick();return;}
...
function renderMode(){
 const mode=state?.room.mode??chosenMode,boom=mode==='boom',crystal=mode==='crystal',estate=mode==='estate';
 ...
 $('#boomRules').classList.toggle('hidden',!boom);$('#zombieRules').classList.toggle('hidden',boom||crystal||estate);$('#crystalRules').classList.toggle('hidden',!crystal);$('#estateRules').classList.toggle('hidden',!estate);
 if(estate){$('#modeName').textContent='เมืองแมวเศรษฐี';$('#tableTitle').textContent='เมืองแมวเศรษฐี';$('#heroTitle').innerHTML='ทอยอีกก้าว.<br>เป็นเจ้าของ<span>ทั้งเมือง.</span>';$('#heroDescription').innerHTML='ซื้อที่ดิน สร้างร้าน เก็บค่าเช่า<br>วางแผนเงินให้ดี แล้วเป็นเศรษฐีแห่ง MuDang';$('#heroCat').src='/assets/estate-cat.svg';$('#heroCat').alt='มูแดง นักธุรกิจเมืองแมว';$('#heroCard').src='/assets/estate-cat.svg';$('#lobbyRuleTitle').innerHTML='เมืองแมว.<br>ใครจะครอง?';$('#lobbyRuleText').innerHTML='ทอยเต๋า ซื้อที่ดิน และสร้างร้าน<br>เก็บค่าเช่าจากเพื่อนที่เดินผ่าน<br>รักษาเงินไว้ อย่าให้ล้มละลาย';$('#lobbyRuleNote').textContent='เริ่ม 1,500 เหรียญ · ผ่านจุดเริ่ม +250 · 30 รอบ';$('#rulesTitle').textContent='วิธีเล่น เมืองแมวเศรษฐี';}
 if(crystal){
